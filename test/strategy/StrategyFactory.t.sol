...
contract StrategyFactoryTest is TestBase {
    ...
    function test_createStrategy() external {
        ...
        factory.createStrategy(address(loopingHelper), pool, WBTC, USDC, 0);
        ...
        vm.expectRevert();
        factory.createStrategy(address(loopingHelper), pool, WBTC, USDC, 1);

        // create a strategy with mbasis/usdc with 0 emode
        factory.createStrategy(address(loopingHelper), pool, MTBILL, USDC, 0);
        ...
        factory.createStrategy(address(loopingHelper), pool, MTBILL, USDC, 1);
        ...
        vm.expectRevert();
        factory.createStrategy(address(loopingHelper), pool, MTBILL, USDC, 2);
